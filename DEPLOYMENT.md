# 🚀 Інструкція з Деплою та Rollback

## 📋 Змінні Оточення для Production

Додай ці змінні в налаштування хостингу:

```env
NEXT_PUBLIC_URL_API=https://taurus.pp.ua
NEXT_PUBLIC_TELEGRAM_BOT_NAME=EridonKharkiv_bot
NEXT_PUBLIC_FORCE_BROWSER_AUTH=true
```

### Vercel
1. Project Settings → Environment Variables
2. Додай кожну змінну окремо
3. Environment: **Production**
4. Redeploy проєкт

### Railway/Render
1. Settings → Variables
2. Додай всі 4 змінні
3. Redeploy автоматично

## 🔄 Rollback Стратегія

### Швидкий откат (Vercel)
1. Deployments → знайди попередній успішний деплой
2. Три крапки → **Promote to Production**
3. Готово за 30 секунд! ✅

### Git Rollback
```bash
# Повернутися до стабільної версії
git checkout v1.0-before-refactor

# Або скасувати останній коміт
git revert HEAD

# Push і автоматичний redeploy
git push origin main
```

## ✅ Перевірка після деплою

Відкрий консоль браузера (F12) і перевір:
- ✅ Немає помилок 401/403
- ✅ Є лог `[Axios Interceptor] Found initData` або `Found accessToken`
- ✅ API запити виконуються успішно

## 🛡️ Backup створено

Tag: `v1.0-before-refactor` - стабільна версія перед рефакторингом

## 📝 Що змінилося

- Видалено 122+ передач `initData`
- Конвертовано 10 серверних компонентів на клієнтські
- Всі API запити тепер через axios interceptor
- Підтримка двох режимів: Telegram Mini App + Browser login

## ⚠️ Важливо

- **НЕ додавай** `NEXT_PUBLIC_DEV=true` на прод
- Переконайся, що `https://taurus.pp.ua` доступний
- Після додавання змінних - **обов'язково redeploy**

## 🔧 Налаштування Backend (CORS)

**Важливо!** Додай домен Vercel до CORS на бекенді:

```python
# backend/main.py (або де у тебе FastAPI app)
origins = [
    "http://localhost",
    "http://localhost:80",
    "https://telegram-mini-app-six-inky.vercel.app",  # Твій Vercel домен
]

app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
```

**Або використай wildcard для всіх Vercel доменів:**
```python
allow_origins=["*"]  # Для тестування (не рекомендується для проду)
```
