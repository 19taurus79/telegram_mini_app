export function getInitData(): string {
  ...
  if (typeof window !== "undefined" && window.Telegram?.WebApp?.initData) {
    console.log("getInitData: Real Telegram Mini App detected.");
    return window.Telegram.WebApp.initData;
  }
  
  // Принудительно используем браузерную аутентификацию
  if (forceBrowserAuth) {
    console.log("getInitData: FORCE_BROWSER_AUTH is true. Returning empty string for browser mode.");
    return "";
  }
  
  if (isDev && typeof window !== "undefined" && (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1")) {
    console.log("getInitData: Local development detected. Returning test user data.");
  ...
  console.log("getInitData: No Telegram WebApp and not in DEV mode. Browser mode defaults.");
  return "";
}
