...
function Header() {
  ...
  // Удаляем useEffect, который устанавливал initData
  // useEffect(() => {
  ...
  // Удаляем useEffect, который синхронизировал useUser с useAuthStore
  // useEffect(() => {
  //   if (authUser) {
    ...
    <header className={css.header} ref={headerRef}>
      ...
      <nav className={`${css.nav} ${menuOpen ? css.navOpen : ""}`}>
        <ul className={css.navList}>
          ...
          {authUser?.is_admin && ( // Используем authUser напрямую
            <>
              <li>
